...
describe("Signup with token", () => {
  ...
  const getBuilderWithAuth = getBuilderWithAuthUseCase(dependencies).execute;

  test("Should get data with token email connexion", async () => {
    ...
    const builderDataToken = await getBuilderWithAuth(builder._id);
